...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        setError("No se pudieron cargar las estadísticas. Verifica tu conexión o sesión.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const statsConfig = [
    { label: 'Materias Activas', value: data.materias_activas, icon: BookOpen, color: 'bg-blue-500' },
    { label: 'Grupos Totales', value: data.grupos_totales, icon: Users, color: 'bg-purple-500' },
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center h-64 text-gray-500">
        ...
        <p className="animate-pulse">Obteniendo datos de IT Celaya...</p>
      </div>
    );
  }
  ...
  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
      <div className="mb-8">
        ...
        <p className="text-gray-500">Resumen actual de tu progreso académico.</p>
      </div>

      <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4 mb-8">
        {statsConfig.map((item) => (
          <div key={item.label} className="bg-white overflow-hidden shadow-sm rounded-xl border border-gray-100 hover:shadow-md transition-all duration-300 transform hover:-translate-y-1">
            <div className="p-5">
      ...
      <div className="bg-white shadow-sm rounded-xl border border-gray-100 p-6">
        ...
        {data.proximas_exposiciones && data.proximas_exposiciones.length > 0 ? (
          <div className="space-y-4">
            {data.proximas_exposiciones.map((expo, index) => (
              <div key={index} className="flex items-center p-4 border border-gray-50 rounded-xl hover:bg-gray-50 transition-colors border-l-4 border-l-green-500">
          ...
          <div className="text-center py-12 text-gray-400">
            ...
            <p className="text-sm">No tienes exposiciones registradas en tus equipos actuales.</p>
          </div>
        )}
      </div>
    </div>
